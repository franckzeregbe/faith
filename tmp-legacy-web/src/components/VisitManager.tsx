...
export default function VisitManager() {
  ...
  useEffect(() => {
    const result = loadValidatedList(STORAGE_KEYS.visits, visitSchema)
    setVisits(result.data)
    requestPermission()
    checkAllNotifications(result.data)
  }, [])

  useEffect(() => {
    const result = saveValidatedList(STORAGE_KEYS.visits, visitSchema, visits)
