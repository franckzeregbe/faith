...
import { hashString, loadJsonFromStorage, isNumericPin, isHashedPin, STORAGE_KEYS } from '../utils/storage'
...
export default function PinLock({ onUnlock }: Props) {
  ...
  useEffect(() => {
    const pin = loadJsonFromStorage<string>(STORAGE_KEYS.pin, '')
    if (pin && isNumericPin(pin)) {
      hashString(pin).then(hashed => {
        setStoredPin(hashed)
        localStorage.setItem(STORAGE_KEYS.pin, JSON.stringify(hashed))
