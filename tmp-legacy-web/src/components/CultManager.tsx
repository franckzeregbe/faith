...
import { cultSchema } from '../schemas'
...
export default function CultManager() {
  ...
  useEffect(() => {
    const result = loadValidatedList(STORAGE_KEYS.cults, cultSchema)
    setCults(result.data)
    requestPermission()
  }, [])

  useEffect(() => {
    const result = saveValidatedList(STORAGE_KEYS.cults, cultSchema, cults)
